	...
	public sealed class NatNetOptiTrackClient : IOptiTrackClient {
		...
		private void FetchDataDescriptions() {
			...
			for ( int i = 0; i < dataDescriptors.Count; i++ ) {
				...
				switch ( descriptor.type ) {
					...
					skeletons.Add( (Skeleton) descriptor );
					break;

					case (int) DataDescriptorType.eForcePlateData:
					forcePlates.Add( (ForcePlate) descriptor );
					break;

					default:
					statusMessages.Add( "Error: Invalid Data Set" );
					break;
				}
			}
		}
		...
		private void OnFrameReady( FrameOfMocapData data, NatNetClientML client ) {
			...
			try {
				...
				lock ( syncRoot ) {
					if ( data.bTrackingModelsChanged == true || data.nRigidBodies != rigidBodies.Count || data.nSkeletons != skeletons.Count || data.nForcePlates != forcePlates.Count || assetsChanged ) {
						assetsChanged = false;
						FetchDataDescriptions();
					}
