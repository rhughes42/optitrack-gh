...
namespace Tracker.Components {
	...
	public sealed class LoadOptiTrackRecordingComponent : GH_Component {
		...
		public LoadOptiTrackRecordingComponent() : base(
				...
				"LoadRec",
				"Load a recorded OptiTrack JSON file for replay without Motive.",
				"Tracker",
				"Recording") { }
		...
		protected override void SolveInstance(IGH_DataAccess DA) {
			...
			TelemetryContext  context   = new TelemetryContext().SetTag("component", "load_recording").SetTag("format", "json");
			...
			using (telemetry.StartSpan("load_recording_json", context)) {
				try {
					...
					string summary = string.Format(
							"Version {0}, Source {1}, Units {2}, Frames {3}",
					...
					telemetry.CaptureException(
							exception,
							new TelemetryContext().SetTag("operation", "load_recording_json").SetTag("error_type", exception.GetType().Name));

					AddRuntimeMessage(
							GH_RuntimeMessageLevel.Error,
							"Failed to load recording JSON. Confirm the file exists and matches the Tracker recording format.");
