...
namespace Tracker.Components {
	...
	public sealed class RecordOptiTrackStreamComponent : GH_Component {
		...
		public RecordOptiTrackStreamComponent()
			: base( "Record OptiTrack Stream", "RecordRec", "Record OptiTrack frame objects to JSON for replay and offline testing.", "Tracker", "Recording" ) {
		}
					...
					using ( telemetry.StartSpan( "save_recording_json", context ) ) {
						try {
							...
							telemetry.CaptureException( exception, new TelemetryContext()
								.SetTag( "operation", "save_recording_json" )
