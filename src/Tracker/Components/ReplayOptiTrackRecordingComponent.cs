...
namespace Tracker.Components {
	...
	public sealed class ReplayOptiTrackRecordingComponent : GH_Component {
		...
		public ReplayOptiTrackRecordingComponent()
			: base( "Replay OptiTrack Recording", "ReplayRec", "Replay recorded OptiTrack frames without a live Motive connection.", "Tracker", "Recording" ) {
		}
		...
		protected override void SolveInstance( IGH_DataAccess DA ) {
			...
			TelemetryContext context = new TelemetryContext()
				.SetTag( "component", "replay_recording" )
				.SetTag( "format_version", recording.FormatVersion )
				...
				try {
					...
					lock ( sync ) {
						if ( !ReferenceEquals( recording, lastRecording ) ) {
							...
							telemetry.CaptureMessage( "replay_loaded", TelemetrySeverity.Info, new TelemetryContext()
								.SetTag( "operation", "replay_load" )
								.SetTag( "format_version", recording.FormatVersion )
								.SetMetric( "frame_count", recording.Frames.Count ) );
						}
					...
					telemetry.CaptureException( exception, new TelemetryContext()
						.SetTag( "operation", "replay_component" )
		...
		private void EnsureHandlers() {
			...
			replayClient.FrameReceived += ( sender, args ) => {
